...
static void test_init_lcd(void)
{
    ...
    ESP_LOGI(TAG, "Install LCD driver of ota7290b");
    esp_lcd_dpi_panel_config_t dpi_config = OTA7290B_480_1920_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_ota7290b(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test ota7290b to draw pattern with MIPI interface", "[ota7290b][draw_pattern]")
{
...
TEST_CASE("test ota7290b to draw color bar with MIPI interface", "[ota7290b][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test ota7290b to rotate with MIPI interface", "[ota7290b][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
