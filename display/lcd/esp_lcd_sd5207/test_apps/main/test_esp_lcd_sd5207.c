...
#define TEST_LCD_H_RES                  (568)
#define TEST_LCD_V_RES                  (1210)
#define TEST_LCD_BIT_PER_PIXEL          (24)
#define TEST_PIN_NUM_LCD_RST            (32)
...
static void test_init_lcd(void)
{
    ...
    esp_lcd_dsi_bus_config_t bus_config = SD5207_PANEL_BUS_DSI_2CH_CONFIG();
    TEST_ESP_OK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus));

    ESP_LOGI(TAG, "Install panel IO");
    esp_lcd_dbi_io_config_t dbi_config = SD5207_PANEL_IO_DBI_CONFIG();
    TEST_ESP_OK(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &mipi_dbi_io));

    ESP_LOGI(TAG, "Install LCD driver of sd5207");
    esp_lcd_dpi_panel_config_t dpi_config = SD5207_568_1210_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_sd5207(mipi_dbi_io, &panel_config, &panel_handle));
...
TEST_CASE("test sd5207 to draw pattern with MIPI interface", "[sd5207][draw_pattern]")
{
...
TEST_CASE("test sd5207 to draw color bar with MIPI interface", "[sd5207][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test sd5207 to rotate with MIPI interface", "[sd5207][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
