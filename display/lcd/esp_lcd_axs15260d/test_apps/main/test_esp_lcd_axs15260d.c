...
static char *TAG = "axs15260d_test";
...
static void test_init_lcd(void)
{
    ...
    esp_lcd_dsi_bus_config_t bus_config = AXS15260D_PANEL_BUS_DSI_2CH_CONFIG();
    TEST_ESP_OK(esp_lcd_new_dsi_bus(&bus_config, &mipi_dsi_bus));

    ESP_LOGI(TAG, "Install panel IO");
    esp_lcd_dbi_io_config_t dbi_config = AXS15260D_PANEL_IO_DBI_CONFIG();
    TEST_ESP_OK(esp_lcd_new_panel_io_dbi(mipi_dsi_bus, &dbi_config, &mipi_dbi_io));

    ESP_LOGI(TAG, "Install LCD driver of axs15260d");
    esp_lcd_dpi_panel_config_t dpi_config = AXS15260D_452_1280_PANEL_60HZ_DPI_CONFIG(TEST_MIPI_DPI_PX_FORMAT);
    ...
    TEST_ESP_OK(esp_lcd_new_panel_axs15260d(mipi_dbi_io, &panel_config, &panel_handle));
...
{
    uint8_t byte_per_pixel = (TEST_LCD_BIT_PER_PIXEL + 7) / 8; // 3:rgb888 2:rgb565
    uint16_t row_line = v_res / byte_per_pixel / 8;//48
    uint8_t *color = (uint8_t *)heap_caps_calloc(1, row_line * h_res * byte_per_pixel, MALLOC_CAP_DMA);
    if (!color) {
        ESP_LOGE("TAG", "Failed to allocate memory for red screen");
        return;
    }
...
TEST_CASE("test axs15260d to draw pattern with MIPI interface", "[axs15260d][draw_pattern]")
{
...
TEST_CASE("test axs15260d to draw color bar with MIPI interface", "[axs15260d][draw_color_bar]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
TEST_CASE("test axs15260d to rotate with MIPI interface", "[axs15260d][rotate]")
{
    ESP_LOGI(TAG, "Initialize LCD device");
    test_init_lcd();
...
{
    /**
    *         _     __  __   ____    _   ____    ____     __      ___        _ 
    *        / \    \ \/ /  / ___|  / | | ___|  |___ \   / /_    / _ \    __| |
    *       / _ \    \  /   \___ \  | | |___ \    __) | | '_ \  | | | |  / _` |
    *      / ___ \   /  \    ___) | | |  ___) |  / __/  | (_) | | |_| | | (_| |
    *     /_/   \_\ /_/\_\  |____/  |_| |____/  |_____|  \___/   \___/   \__,_|
    *                                                                          
    */
    unity_run_menu();
}
